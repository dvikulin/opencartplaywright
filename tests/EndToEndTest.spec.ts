...
async function addProductToCart(page: Page) {
    ...
    // Perform search using the more reliable Enter-based method
    ...
    // Validate desired product exists
...
async function verifyShoppingCart(page: Page) {
    ...
    const actualQuantity = await shoppingCartPage.getCartQuantity();
    const actualTotal = await shoppingCartPage.getTotalPrice();

    console.log("Cart quantity:", actualQuantity);
    console.log("Cart total:", actualTotal);

    // Validate quantity
    expect(actualQuantity).toBe(config.productQuantity);

    // Validate total price
    expect(actualTotal).toBe(config.totalPrice);
}
